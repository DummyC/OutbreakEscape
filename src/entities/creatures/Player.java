...
public class Player extends Creature {
    ...
    public Player(Vector2D position, double maxVelocity, World world) {
        ...
        this.world = world;
        this.lighting = new Lighting(world, 1000);
        ...
        hitMarshal = new Sound(Assets.hitMarshal);
        pickup = new Sound(Assets.pickup);

        spawnMarshal1 = new Sound(Assets.spawnMarshal1);
        spawnMarshal2 = new Sound(Assets.spawnMarshal2);
        spawnMarshal3 = new Sound(Assets.spawnMarshal3);

        gameOver = new Sound(Assets.gameOver);

        shootPoint = new Vector2D();

        pistol = new Gun(Assets.pistolSkin, pistolIdle, pistolReload, pistolShoot, pistolShootSound, pistolReloadSound, this, 700, 9, 18);
        rifle = new Gun(Assets.rifleSkin, rifleIdle, rifleReload, rifleShoot, rifleShootSound, rifleReloadSound, this, 100, 30, 90);

        currentGun = pistol;
        sprint = velocity.getX() + 2;
    }
    ...
    public void update() {
        ...
        if (KeyManager.sprint) {
            velocity.setX(sprint);
            velocity.setY(sprint);
        } else {
        ...
        if (health <= 0) {
            world.gameState.gameState = world.gameState.gameOverState;
        }
    ...
    public Entity collision(double xMove, double yMove) {
        ...
        for (int i = 0; i < world.getObjects().size(); i++) {
            Entity e = world.getObjects().get(i);

            if (e.getDimensions(0, 0).intersects(this.getDimensions(xMove, yMove))) {
                pickup.playSound();
                if (e.type.equals("ammo")) {
                    rifle.addBullets(50);
                    pistol.addBullets(15);
                } else if (e.type.equals("health")) {
                    health += 100;
                    if (health > MAXHEALTH) {
                        health = MAXHEALTH;
                    }
    ...
    public void render(Graphics g) {
        ...
        g.setFont(Assets.secondary);
        g.setFont(g.getFont().deriveFont(Font.PLAIN, 25));
        g.drawImage(Assets.hudTop, 0, 0, null);
        g.drawString("Points: " + world.points, 540, 40);
        ...
        g2d.fillRect(25, 25, health, 25);

    }

    public void setDefaults() {
        health = 400;
        pistol.setBullets(18);
        rifle.setBullets(90);
        pistol.setRound(9);
        rifle.setRound(30);
    ...
    public void spawn() {
        ...
        switch (randomNum) {
            case 1:
                spawnMarshal1.playSound();
                break;
            case 2:
                spawnMarshal2.playSound();
                break;
            case 3:
                spawnMarshal3.playSound();
                break;
        }
